// Waits for the FDC voting round to finalize, then polls the Data Availability
// Layer for the Merkle proof + abi-encoded IWeb2Json.Response.
import { ethers } from "ethers";
import {
  env,
  CONTRACT_REGISTRY_ADDRESS,
  CONTRACT_REGISTRY_ABI,
  FDC_VERIFICATION_WEB2JSON_ABI,
  RELAY_ABI,
  sleep,
  loadPhase2State,
  savePhase2State,
...
async function main() {
  ...
  if (!state.abiEncodedRequest || state.votingRoundId === undefined) {
    throw new Error("Missing abiEncodedRequest/votingRoundId in state.phase2.json — run 05-submit-attestation.mjs first.");
  }
  ...
  const fdcVerification = new ethers.Contract(fdcVerificationAddress, FDC_VERIFICATION_WEB2JSON_ABI, provider);
  const relay = new ethers.Contract(relayAddress, RELAY_ABI, provider);

  // fdcProtocolId isn't in the Web2Json-only ABI subset above; resolve it via
  // a minimal separate call rather than widening that ABI's purpose.
  const protocolIdIface = new ethers.Contract(fdcVerificationAddress, ["function fdcProtocolId() external view returns (uint8)"], provider);
  const protocolId = await protocolIdIface.fdcProtocolId();
  ...
  await savePhase2State({ proof, fdcVerificationAddress });
  console.log("\nSaved proof to state.phase2.json");
}
