...
async function main() {
  ...
  if (!state.abiEncodedRequest) {
    throw new Error("Missing abiEncodedRequest in state.json — run 03-prepare-attestation.mjs first.");
  }

  const { COSTON2_RPC_URL, COSTON2_PRIVATE_KEY } = env;
  if (!COSTON2_PRIVATE_KEY) {
    throw new Error("Missing COSTON2_PRIVATE_KEY — run 01-generate-accounts.mjs and fund it first.");
  }
  ...
  await saveState({ fdcRequestTxHash: tx.hash, votingRoundId: roundId });
  console.log("\nSaved fdcRequestTxHash and votingRoundId to state.json");
}
