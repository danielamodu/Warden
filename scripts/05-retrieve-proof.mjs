...
// Layer for the Merkle proof + abi-encoded IPayment.Response.
...
async function main() {
  ...
  if (!state.abiEncodedRequest || state.votingRoundId === undefined) {
    throw new Error("Missing abiEncodedRequest/votingRoundId in state.json — run 04-submit-attestation.mjs first.");
  }
  ...
  await saveState({ proof });
  console.log("\nSaved proof to state.json");
}
