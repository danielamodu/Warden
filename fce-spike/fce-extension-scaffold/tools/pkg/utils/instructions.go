...
// FetchTeePublicKeyForEncryption fetches the deployed TEE machine's public key
// from the extension proxy's /info endpoint and parses it for use as an ECIES
// encryption target. This is the same public key the on-chain
// TeeMachineRegistry has registered for this extension's machine — encrypting
// to it means only that specific enclave can decrypt (via its local
// /decrypt endpoint), not the proxy, not the chain, not an observer of the
// public transaction that carries the ciphertext.
func FetchTeePublicKeyForEncryption(proxyURL string) (*ecies.PublicKey, error) {
	teeInfo, err := fccutils.TeeInfo(proxyURL)
	if err != nil {
		return nil, errors.Errorf("fetching TEE info from proxy: %s", err)
	}
...
// EncryptCheckGreaterThan10Value JSON-encodes {"value": N} and ECIES-encrypts
// it to the TEE's public key. The plaintext value never leaves this function
// unencrypted — the returned ciphertext is what actually goes into the
// on-chain transaction's calldata.
func EncryptCheckGreaterThan10Value(teePubKey *ecies.PublicKey, value int) ([]byte, error) {
	plaintext, err := json.Marshal(map[string]int{"value": value})
	if err != nil {
...
// SendCheckGreaterThan10 submits an already-ECIES-encrypted value on-chain.
// The caller is responsible for encrypting via EncryptCheckGreaterThan10Value
// first — this function only ever sees ciphertext, and so does the chain.
func SendCheckGreaterThan10(s *support.Support, instructionSenderAddress common.Address, encryptedValue []byte) (common.Hash, common.Hash, error) {
	sender, err := helloworld.NewHelloWorldInstructionSender(instructionSenderAddress, s.ChainClient)
	if err != nil {
		return common.Hash{}, common.Hash{}, errors.Errorf("failed to bind contract: %s", err)
	}
	...
	tx, err := sender.SendCheckGreaterThan10(opts, encryptedValue)
	if err != nil {
		reason := fccutils.DecodeRevertReason(err)
		if reason == "" {
			parsed, _ := helloworld.HelloWorldInstructionSenderMetaData.GetAbi()
			if parsed != nil {
				callData, packErr := parsed.Pack("sendCheckGreaterThan10", encryptedValue)
				if packErr == nil {
					from := crypto.PubkeyToAddress(s.Prv.PublicKey)
					reason = fccutils.SimulateAndDecodeRevert(
						s.ChainClient, from, instructionSenderAddress,
